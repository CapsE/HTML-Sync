...
imported.src=Part.includes[i];else{if(!endsWith(Part.includes[i],".css")){console.debug("Couldn't load "+Part.includes[i]);continue}imported=document.createElement("link"),imported.href=Part.includes[i],imported.type="text/css",imported.rel="stylesheet"}imported.dataset.id=this.id,imported.addEventListener("load",function(a){HTMLSync.parts[this.dataset.id].includeLoaded()}),document.head.appendChild(imported)}else newElement.addEventListener("added",function(a){HTMLSync.parts[this.id].includeLoaded()});for(var i in this.content)this.content[i].renderHTML(newElement);target.appendChild(newElement)},Part.prototype.changeId=function(a,b){var c=!1;b||(console.debug("Main ID is "+this.id),c=!0,b=this.id);for(var d in this.content){d=this.content[d];var e=d.id.split("_");e[0]==b&&(console.debug(d.id+" needs update"),c=!0,d.id=a+"_"+e[1]),d.changeId(a,b)}if(c)for(var f in this.functions)for(var g in this.functions[f]){console.debug("Changing function "+g+" of all "+f+" functions for Object "+this.id),console.debug("Replacing "+b+" with "+a);var h=new RegExp(b,"g");this.functions[f][g]=this.functions[f][g].replace(h,a)}this.id==b&&(this.id=a)},Part.includes=[],Part.includesToLoad=0,Part}(Syncable);
